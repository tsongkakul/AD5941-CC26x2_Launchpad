 ...
 */
AppCHRONOAMPCfg_Type AppCHRONOAMPCfg = { .bParaChanged = bFALSE, .SeqStartAddr =
                                                 0,
                                         .MaxSeqLen = 0,
...
AD5940Err AppCHRONOAMPCtrl(int32_t AmpCtrl, void *pPara)
{
    ...
    case CHRONOAMPCTRL_START:
    {
        ...
        wupt_cfg.SeqxWakeupTime[SEQID_0] =
                (uint32_t) (AppCHRONOAMPCfg.WuptClkFreq * AppCHRONOAMPCfg.AmpODR)
                        - 2 - 1;
...
static AD5940Err AppCHRONOAMPSeqCfgGen(void)
{
    ...
    lp_loop.LpDacCfg.DacData6Bit = (uint32_t) ((AppCHRONOAMPCfg.Vzero - 200)
            / DAC6BITVOLT_1LSB);
    lp_loop.LpDacCfg.DacData12Bit = (int32_t) ((AppCHRONOAMPCfg.SensorBias)
    ...
    if (error == AD5940ERR_OK)
    {
        ...
        AD5940_SEQCmdWrite(AppCHRONOAMPCfg.InitSeqInfo.SeqRamAddr, pSeqCmd,
                           SeqLen);
    }
    else
        return error; /* Error */
...
static AD5940Err AppCHRONOAMPTransientMeasureGen(void)
{
    ...
    VbiasCode = (int32_t) ((AppCHRONOAMPCfg.pulseAmplitude
            + AppCHRONOAMPCfg.SensorBias) / DAC12BITVOLT_1LSB) + VzeroCode * 64;
    ...
    clks_cal.RatioSys2AdcClk = AppCHRONOAMPCfg.SysClkFreq
            / AppCHRONOAMPCfg.AdcClkFreq;
    ...
    AD5940_WriteReg(
            REG_AFE_LPDACDAT0,
            (uint32_t) ((AppCHRONOAMPCfg.Vzero - 200) / DAC6BITVOLT_1LSB) << 12
                    | (int32_t) ((AppCHRONOAMPCfg.SensorBias)
    ...
    if (error == AD5940ERR_OK)
    {
        ...
        AppCHRONOAMPCfg.TransientSeqInfo.SeqRamAddr =
                AppCHRONOAMPCfg.MeasureSeqInfo.SeqRamAddr
                        + AppCHRONOAMPCfg.MeasureSeqInfo.SeqLen;
        AppCHRONOAMPCfg.TransientSeqInfo.pSeqCmd = pSeqCmd;
        AppCHRONOAMPCfg.TransientSeqInfo.SeqLen = SeqLen;
        /* Write command to SRAM */
        AD5940_SEQCmdWrite(AppCHRONOAMPCfg.TransientSeqInfo.SeqRamAddr, pSeqCmd,
                           SeqLen);
    }
    else
        return error; /* Error */
    ...
    if (error == AD5940ERR_OK)
    {
        ...
        AppCHRONOAMPCfg.MeasureSeqInfo.SeqRamAddr =
                AppCHRONOAMPCfg.InitSeqInfo.SeqRamAddr
                        + AppCHRONOAMPCfg.InitSeqInfo.SeqLen;
        AppCHRONOAMPCfg.MeasureSeqInfo.pSeqCmd = pSeqCmd;
        AppCHRONOAMPCfg.MeasureSeqInfo.SeqLen = SeqLen;
        /* Write command to SRAM */
        AD5940_SEQCmdWrite(AppCHRONOAMPCfg.MeasureSeqInfo.SeqRamAddr, pSeqCmd,
                           SeqLen);
    }
    else
        return error; /* Error */
...
static AD5940Err AppCHRONOAMPDataProcess(int32_t * const pData,
                                         uint32_t *pDataCount)
{
...
{
    uint32_t temp = 0;
    const uint32_t sinc2osr_table[] = { 22, 44, 89, 178, 267, 533, 640, 667,
                                        800, 889, 1067, 1333, 0 };
    const uint32_t sinc3osr_table[] = { 5, 4, 2, 0 };

    temp = time * 800 / sinc2osr_table[AppCHRONOAMPCfg.ADCSinc2Osr]
            / sinc3osr_table[AppCHRONOAMPCfg.ADCSinc3Osr];
...
float AppCHRONOAMPCalcVoltage(uint32_t ADCcode)
{
    ...
    {
    case ADCPGA_1:
        fVolt = ((float) (tmp) / 32768) * (AppCHRONOAMPCfg.ADCRefVolt / 1)
                * kFactor;
        break;
    case ADCPGA_1P5:
        fVolt = ((float) (tmp) / 32768) * (AppCHRONOAMPCfg.ADCRefVolt / 1.5f)
                * kFactor;
        break;
    case ADCPGA_2:
        fVolt = ((float) (tmp) / 32768) * (AppCHRONOAMPCfg.ADCRefVolt / 2)
                * kFactor;
        break;
    case ADCPGA_4:
        fVolt = ((float) (tmp) / 32768) * (AppCHRONOAMPCfg.ADCRefVolt / 4)
                * kFactor;
        break;
    case ADCPGA_9:
        fVolt = ((float) (tmp) / 32768) * (AppCHRONOAMPCfg.ADCRefVolt / 9)
                * kFactor;
        break;
    }
    return fVolt;
