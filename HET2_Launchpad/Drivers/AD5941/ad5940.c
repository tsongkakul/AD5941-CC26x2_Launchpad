...
static uint16_t AD5940_ReadWrite16B(uint16_t data)
{
   ...
   spiTransaction.txBuf = (void *)SendBuffer;
   spiTransaction.rxBuf = (void *)RecvBuffer;
   SPI_transfer(spi, &spiTransaction);
...
#ifndef CHIPSEL_M355
  AD5940_RstClr();
  AD5940_Delay10us(400); /* Delay some time */
  AD5940_RstSet();
  ...
  */
  ...
  else
  {
    ...
    */
    ...
    pADCBaseCfg = &dsp_cfg.ADCBaseCfg; 	/* DFT on RCAL */
    pADCBaseCfg->ADCMuxN = ADCMUXN_N_NODE;
    pADCBaseCfg->ADCMuxP = ADCMUXP_P_NODE;
    pADCBaseCfg->ADCPga = ADCPgaGainRcal;
    AD5940_ADCBaseCfgS(pADCBaseCfg);
